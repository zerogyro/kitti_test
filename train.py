...
from nets.yolo_nets.yolo_loss import YoloLoss
from nets.yolo_nets.yolov3 import yolo_body

import os
from tensorflow.keras import optimizers, callbacks, metrics
from tensorflow.keras.optimizers.schedules import PolynomialDecay
weight_path = '/home/jerry/PycharmProjects/kitti_work/log/yolov3_110.5882.h5'
...
def train_by_fit(optimizer, loss, train_data, train_steps, validation_data, validation_steps):
    cbk = [
        ...
        callbacks.ModelCheckpoint('./log2/yolov3_{val_loss:.04f}.h5', save_best_only=True, save_weights_only=True)
    ]

    input = tf.keras.layers.Input(shape=(416, 416, 3))
    model = yolo_body(input, 3, cfg.num_classes)
